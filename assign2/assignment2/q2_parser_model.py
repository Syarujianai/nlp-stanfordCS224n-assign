import pickle # python3: instead of cPickle
import sys # python3
import os
import time
...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        embeds = tf.Variable(self.pretrained_embeddings, name="embeds")
        # Refer: https://www.jianshu.com/p/abea0d9d2436
        # input_placeholder: (None, n_features), result: [(None, n_features) | embedding_size], if embed are row vector.
        lookup_embeds = tf.nn.embedding_lookup(embeds, self.input_placeholder, name="lookup_embeds")
        embeddings = tf.reshape(lookup_embeds, shape=(-1, self.config.n_features * self.config.embed_size), name="concat_embeds")
    ...
    def run_epoch(self, sess, parser, train_examples, dev_set):
        ...
        for i, (train_x, train_y) in enumerate(minibatches(train_examples, self.config.batch_size)):
            ...
            prog.update(i + 1, [("train loss", np.pad(loss, (self.config.batch_size - len(loss),0),
                    'constant', constant_values=(0,)) if loss.shape!=(self.config.batch_size,) else loss)])
...
def main(argv):
    ...
    print(80 * "=")
    config = Config()
    parser, embeddings, train_examples, dev_set, test_set = load_and_preprocess_data(FLAGS.debug)
    ...
    with tf.Graph().as_default() as graph:
        ...
        saver = None if FLAGS.debug else tf.train.Saver()
